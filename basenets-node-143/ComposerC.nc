...
//		call Send.output (serviceInfo.provider, serviceInfo.service, numNodes, services[0].domain_size, services[1].domain_size);
...
/*		num_reply++;
		if (num_reply > 2) {
			rt = call SysTime.getTime32 () - rt;
			call Send.output (rt, rt>>8, rt>>16, rt>>24, 44);
		}
*/
...
//		call Send.output (numNodes, services[0].domain_size, services[1].domain_size, services[2].domain_size, node_list[1].id);
...
//		call Send.output (services[0].domain_size, services[3].domain_size, services[4].domain_size, services[0].domain[0], services[4].domain[0]);
...
//	call Send.output (iteration, services[0].solution, services[1].solution, services[2].solution, services[3].solution);
		...
		if (no_solution) {
			call Send.output (1, 1, 2, 3, 5);
			signal ComposerInterface.composed (FAIL);
			return SUCCESS;
		}

//	call Send.output (iteration, services[0].solution, services[1].solution, services[2].solution, numConnections);
