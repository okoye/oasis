...
*/	
	...
	command result_t StdControl.start () {
		if (add (local_services,NOTIFY,ls_size)) 		{			ls_size += 1;		}
		if (add (local_services,READ_TEMP,ls_size)) {			ls_size += 1;		}
		if (add (local_services,LOCALIZE,ls_size)) {			ls_size += 1;		}
		if (add (local_services,WIND,ls_size)) {			ls_size += 1;		}
...
//			call Send.output (ds_size,msg.service,msg.provider,msg.power,cp_size);
			infomsg = msg;
