...
module NodeOutC {
	provides {
		...
		interface ServiceInterface as LocalizeInterface;
	...
	bool add_b (Binding list[], uint8_t name, uint8_t dsts, uint16_t dstn, uint8_t size) {
		uint8_t i = 0;
	...
	uint8_t getIndex_b (Binding list[], uint8_t name, uint8_t size) {
		uint8_t i = 0;
		if (size == 0) {
			return MAX_SERVICES;
		}
	...
	uint8_t buffer_localize[BIGMSG_BUFFER_SIZE];
	uint8_t buffer_notify[BIGMSG_BUFFER_SIZE];
	uint8_t buffer_readt[BIGMSG_BUFFER_SIZE];
	uint8_t buffer_wind[BIGMSG_BUFFER_SIZE];
	...
	uint8_t service_localize_dst;
	uint8_t service_notify_dst;
	uint8_t service_readt_dst;
	uint8_t service_wind_dst;
	...
	command result_t StdControl.start () {
		if (add_b(local_services, LOCALIZE, 0, 0, ls_size)) 	{ ls_size += 1;	}
		if (add_b(local_services, NOTIFY, 0, 0, ls_size)) 		{ ls_size += 1;	}
		if (add_b(local_services, READ_TEMP, 0, 0, ls_size)) 	{ ls_size += 1;	}
		if (add_b(local_services, WIND, 0, 0, ls_size)) 	{ ls_size += 1;	}
 ...
 *
 **/
	command result_t LocalizeInterface.trigger (uint16_t rate) {
		return SUCCESS;
	}

	// receive service message(s) from local service, 'stitch' them together and send it using SendBigMsg
	command result_t LocalizeInterface.output (ServiceMsg message) {
		// TODO :: wait for full message to arrive and put it on send interface
		uint8_t localize_index = getIndex_b (local_services, LOCALIZE, ls_size);
		uint8_t seqNum = message.seqNum;
		SControl ctrl;
		ctrl.src = TOS_LOCAL_ADDRESS;
		ctrl.srcs = LOCALIZE;
		ctrl.dst = local_services[localize_index].dst_node;
		ctrl.dsts = local_services[localize_index].dst_service;
		ctrl.seqnum = 0xFF;
		ctrl.type = 0x0;	// user defined

		if (ctrl.dst == TOS_LOCAL_ADDRESS) {
			return signal InternalInterface.passToken (ctrl.dsts, message);
		} else if (ctrl.dst == 0) {
			return FAIL;
		}
...
//				call SendInterface.send (service_A_dst, message.data + sizeof(totalLength), message.data + totalLength); 
				...
				memcpy (buffer_localize, message.data + sizeof(totalLength), SERVICE_MSG_DATA_LENGTH - sizeof(totalLength));
			...
			memcpy (buffer_localize + currentLength, message.data, SERVICE_MSG_DATA_LENGTH);
			...
			if (currentLength > totalLength) { // this is the last packet of the current big message sequence
				memcpy (send_buffer, &ctrl, sizeof(SControl));
				memcpy (send_buffer+sizeof(SControl), buffer_localize, totalLength);
				call SendInterface.send (ctrl.dst, send_buffer, send_buffer + totalLength + sizeof(SControl)); 
				pending = FALSE;
				totalLength = 0;
			}
		}
		signal LocalizeInterface.outputComplete (SUCCESS);
		return SUCCESS;
	}

	command result_t NotifyInterface.trigger (uint16_t rate) {
		return SUCCESS;
	}
		
	// receive service message(s) from local service, 'stitch' them together and send it using SendBigMsg
	command result_t NotifyInterface.output (ServiceMsg message) {
		// TODO :: wait for full message to arrive and put it on send interface
		uint8_t notify_index = getIndex_b (local_services, NOTIFY, ls_size); 
		uint8_t seqNum = message.seqNum;
		SControl ctrl;
		ctrl.src = TOS_LOCAL_ADDRESS;
		ctrl.srcs = NOTIFY;
		ctrl.dst = local_services[notify_index].dst_node;
		ctrl.dsts = local_services[notify_index].dst_service;
		ctrl.seqnum = 0xFF;
		ctrl.type = 0x0;	// user defined
...
//				call SendInterface.send (service_A_dst, message.data + sizeof(totalLength), message.data + totalLength); 
				...
				memcpy (buffer_notify, message.data + sizeof(totalLength), SERVICE_MSG_DATA_LENGTH - sizeof(totalLength));
				currentLength = (SERVICE_MSG_DATA_LENGTH - sizeof(totalLength));
				pending = TRUE;
			}
		} 
			...
			// old message (continuation of previous message)
			memcpy (buffer_notify + currentLength, message.data, SERVICE_MSG_DATA_LENGTH);
			currentLength = currentLength + SERVICE_MSG_DATA_LENGTH;
			if (currentLength > totalLength) { // this is the last packet of the current big message sequence
				memcpy (send_buffer, &ctrl, sizeof(SControl));
				memcpy (send_buffer+sizeof(SControl), buffer_notify, totalLength);
				call SendInterface.send (ctrl.dst, send_buffer, send_buffer + totalLength + sizeof(SControl)); 
				pending = FALSE;
				totalLength = 0;
			}
		}
		signal NotifyInterface.outputComplete (SUCCESS);
		return SUCCESS;
	}
...
//				call SendInterface.send (service_A_dst, message.data + sizeof(totalLength), message.data + totalLength); 
	...
	command result_t WindInterface.trigger (uint16_t rate) {
		return SUCCESS;
	}

	// receive service message(s) from local service, 'stitch' them together and send it using SendBigMsg
	command result_t WindInterface.output (ServiceMsg message) {
		// TODO :: wait for full message to arrive and put it on send interface
		uint8_t wind_index = getIndex_b (local_services, WIND, ls_size); 
		uint8_t seqNum = message.seqNum;
		SControl ctrl;
		ctrl.src = TOS_LOCAL_ADDRESS;
		ctrl.srcs = WIND;
		ctrl.dst = local_services[wind_index].dst_node;
		ctrl.dsts = local_services[wind_index].dst_service;
		ctrl.seqnum = 0xFF;
		ctrl.type = 0x0;	// user defined
...
//				call SendInterface.send (service_A_dst, message.data + sizeof(totalLength), message.data + totalLength); 
				...
				memcpy (buffer_wind, message.data + sizeof(totalLength), SERVICE_MSG_DATA_LENGTH - sizeof(totalLength));
				currentLength = (SERVICE_MSG_DATA_LENGTH - sizeof(totalLength));
				pending = TRUE;
			}
		} 
			...
			// old message (continuation of previous message)
			memcpy (buffer_wind + currentLength, message.data, SERVICE_MSG_DATA_LENGTH);
			currentLength = currentLength + SERVICE_MSG_DATA_LENGTH;
			if (currentLength > totalLength) { // this is the last packet of the current big message sequence
				memcpy (send_buffer, &ctrl, sizeof(SControl));
				memcpy (send_buffer+sizeof(SControl), buffer_wind, totalLength);
				call SendInterface.send (ctrl.dst, send_buffer, send_buffer + totalLength + sizeof(SControl)); 
				pending = FALSE;
				totalLength = 0;
			}
		}
		signal WindInterface.outputComplete (SUCCESS);
		return SUCCESS;
	}
